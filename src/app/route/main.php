...
$app->post('/api/get/address_balance/{ticker}', function ($ticker) use ($app) {
    $func = $app['Function']->getFunctionByTicker($ticker);
    if ($func) {
        $func = $func[0]['addressBalance'];
        return $func($app);
    }
    pageNotFound($app);
});
...
$app->post('/api/walletnotify/{ticker}', function ($ticker) use ($app) {
    $txid = $app['request']->get('txid',null,null,true);
    $name = $app['request']->get('name',null,null,true);
    return walletNotify($txid, $name, $app);
});
...
$app->post('/api/wallet/get_addresses/{ticker}', function ($ticker) use ($app) {
    $func = $app['Function']->getFunctionByTicker($ticker);
    if ($func) {
        $func = $func[0]['getWalletList'];
        return $func($app);
    }
    pageNotFound($app);
});
